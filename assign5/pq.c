...
void pq_insert(struct pq* pq, void* item, int priority) {
  ...
  dynarray_set(pq->heap, -1, new_elem);
  struct pq_elem *child = dynarray_get(pq->heap, -1);
   ...
   */
  int child_idx = child->priority;
  int parent_idx = ((child_idx-1)/2);
  struct pq_elem* parent = dynarray_get(pq->heap, parent_idx);
  while(child->priority != 0){ 
    if(child->item < parent->item){
       void* temp = parent->item;
       child->item = parent->item;
       parent->item = temp;
       child = parent; //new element now points to its parent
       parent = dynarray_get(pq->heap, ((parent_idx-1)/2)); //parent now points to its own parent
    }
  }
}
...
void* pq_first(struct pq* pq) {
  ...
  first_elem = dynarray_get(pq->heap, 0); //zero should be the first right?
...
void* pq_remove_first(struct pq* pq) {
   ...
   * value there in first_elem.
   */
  struct pq_elem *parent = dynarray_get(pq->heap, 0);
  //void* first_val = first_elem->item;

  struct pq_elem *last_elem = dynarray_get(pq->heap, -1);
  
  first_elem->item = last_elem->item;
  dynarray_remove(pq->heap, -1);

  while(pq->heap != NULL){
     if(dynarray_size(pq->heap) >= 1){
	int idx1 = (2 * parent->priority + 1);
        int idx2 = (2 * parent->priority + 2);
        struct pq_elem *child1 = dynarray_get(pq->heap, idx1);
	struct pq_elem *child2 = dynarray_get(pq->heap, idx2);
	if(child1->item < child2->item){
	   if(child1->item < parent->item){
	      //swap
	      void *temp = child1->item;
	      child1->item = parent->item;
	      parent->item = temp;
	   ...
	   if(child2->item < parent->item){
	     
	      void *temp = child2->item;
	      child2->item = parent->item;
	      parent->item = temp;
