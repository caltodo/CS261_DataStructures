...
struct data * findNode(struct Node *tree, int val){
  ...
  */
  //printf("val: %d\n", val);
  struct data *temp_val = malloc(sizeof(struct data*));
  temp_val->number = val; 
  ...
  while(tree != NULL){
    ...
    else if(compare(com->val, temp->val) == -1){
      if(temp->left != NULL){
        temp = temp->left;
      }
      else{
    ...
    else if(compare(com->val, temp->val) == 0){
      free(temp_val);
      free(com);
      return (struct data*)temp->val;
    }
  }
  free(com);
  free(temp_val); 
...
int countNodes(struct Node *tree){
  ...
  int count = 1;
/* uses recursion to increment count by one each time it interacts with a node*/
  if(tree == NULL)
     return 0;
  else{
     if(tree->left != NULL){
...
int secondLargestNode(struct Node *tree){
  ...
  */
  ...
  //to check if the tree has at least 2 nodes
  ...
  /*this loop finds the right most node, if it hits a leaf, it returns its parent. If it hits a node
   that only has a left node it will increment, and then find the rightmost node of that subtree
   and returns it*/
	...
	while(temp->right != NULL)
	   temp = temp->right;
	return temp->val->number;
     }
     return prev->val->number;
  }
}
