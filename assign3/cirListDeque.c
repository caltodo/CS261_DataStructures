...
void queueListEnqueue(struct cirListDeque* q, TYPE item) {
   ...
   */
   //struct DLink *new_link = _createLink(item); 

   /*new_link->next = q->Sentinel->next;
   new_link->prev = q->Sentinel->next->prev;
   q->Sentinel->next->prev = new_link;
   */
...
TYPE queueListDequeue(struct cirListDeque* q) {
   ...
   * the dequeued value.
   */
  int head_val = q->Sentinel->next->value;
  /*struct DLink *temp = q->Sentinel->next;
  q->Sentinel->next->next->prev = q->Sentinel->next->prev;
  q->Sentinel->next->prev->next = q->Sentinel->next;
...
void stackListPush(struct cirListDeque* q,TYPE item) {
   ...
   */
   ...
   new_link->next = q->Sentinel->next;
   new_link->prev = q->Sentinel->next->prev;
   q->Sentinel->next->prev->next = new_link;
   q->Sentinel->next->prev = new_link;
   q->Sentinel = new_link;*/
...
TYPE stackListPop(struct cirListDeque* q) {
  ...
  /*struct DLink *temp = q->Sentinel->next;
  q->Sentinel->next->next->prev = q->Sentinel->next->prev;
  q->Sentinel->next->prev->next = q->Sentinel->next->next;
